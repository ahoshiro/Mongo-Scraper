var axios = require('axios'); // Node Dependencies
var articleQuery = function(topic, beginYear, endYear){ // NY Times API Request Function
...
const apiSave = function(articleObj){
  ...
  const apiURL = window.location.origin + '/api/saved';
...
const apiDelete = function(deleteArticleId){
  ...
  // Get API Post URL (this allows it to work in both localhost and heroku)
  const apiURL = window.location.origin + '/api/delete/' + deleteArticleId;
